...
/**
 * Base class for creation of Prototypers. Creation of new {@link IPrototype}
 * should look like MyPrototyper.newPrototype(...)
 *
 * @param <T>
 */
        ...
        public String toPropertyName(Method method, Object[] args) {
            ...
            if (methodName.startsWith(prefix) && args.length == requiredAttrTypes.length) {
                for (int i = 0; i < args.length; i++) {
                    if (args != null && !(requiredAttrTypes[i].isInstance(args[i]) || args[i] == null)) {
                        return null;
                    }
                }
...
//                        e.printStackTrace();
    ...
    /**
     * Creation of instance from proxy. In realizaion of this method only
     * creation is required: properties will be copied automatically.
     *
     * @param proxy
