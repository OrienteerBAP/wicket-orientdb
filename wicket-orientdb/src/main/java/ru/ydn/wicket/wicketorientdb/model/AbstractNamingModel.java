...
/**
 * {@link IModel}&lt;{@link String}&gt; for obtaining name for particular
 * objects. Model calculate resource key for passed object and then return
 * either key value from resources or buitified version of resource key.
 *
 * @param <T>
 */
public abstract class AbstractNamingModel<T> extends AbstractReadOnlyModel<String> implements IComponentAssignedModel<String> {
    ...
    public String getObject(Component component) {
        ...
        if (defaultValue == null) {
            defaultValue = Strings.lastPathComponent(resourceKey, '.');
        }
        if (defaultValue != null) {
            defaultValue = buitify(defaultValue);
        }
